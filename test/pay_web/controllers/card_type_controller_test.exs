defmodule PayWeb.CardTypeControllerTest do
  ...
  describe "index" do
    test "lists all card_types", %{conn: conn} do
      conn = get(conn, Routes.payments_card_type_path(conn, :index))
  ...
  describe "create card_type" do
    test "renders card_type when data is valid", %{conn: conn} do
      conn = post(conn, Routes.payments_card_type_path(conn, :create), card_type: @create_attrs)
      assert %{"id" => id} = json_response(conn, 201)["data"]

      conn = get(conn, Routes.payments_card_type_path(conn, :show, id))
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, Routes.payments_card_type_path(conn, :create), card_type: @invalid_attrs)
  ...
  describe "update card_type" do
    ...
    test "renders card_type when data is valid", %{
      ...
      conn =
        put(conn, Routes.payments_card_type_path(conn, :update, card_type),
          card_type: @update_attrs
        )
  ...
  describe "delete card_type" do
    ...
    test "deletes chosen card_type", %{conn: conn, card_type: card_type} do
      conn = delete(conn, Routes.payments_card_type_path(conn, :delete, card_type))
      assert response(conn, 204)

      assert_error_sent 404, fn ->
        get(conn, Routes.payments_card_type_path(conn, :show, card_type))
      end
    end
  end
