defmodule PayWeb.PermissionControllerTest do
  ...
  describe "index" do
    test "lists all permissions", %{conn: conn} do
      conn = get(conn, Routes.services_permission_path(conn, :index))
  ...
  describe "create permission" do
    test "renders permission when data is valid", %{conn: conn} do
      conn = post(conn, Routes.services_permission_path(conn, :create), permission: @create_attrs)
      assert %{"id" => id} = json_response(conn, 201)["data"]

      conn = get(conn, Routes.services_permission_path(conn, :show, id))
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn =
        post(conn, Routes.services_permission_path(conn, :create), permission: @invalid_attrs)
  ...
  describe "update permission" do
    ...
    test "renders permission when data is valid", %{
      ...
      conn =
        put(conn, Routes.services_permission_path(conn, :update, permission),
  ...
  describe "delete permission" do
    ...
    test "deletes chosen permission", %{conn: conn, permission: permission} do
      conn = delete(conn, Routes.services_permission_path(conn, :delete, permission))
      assert response(conn, 204)

      assert_error_sent 404, fn ->
        get(conn, Routes.services_permission_path(conn, :show, permission))
      end
    end
  end
