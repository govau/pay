defmodule Soap.Request.Params do
  ...
  # OUR_CHANGE document: 1
  import XmlBuilder, only: [element: 3, generate: 2]
  ...
  # @spec validate_params(params :: {:cdata, term()}) :: tuple()
  def validate_params({:cdata, cdata} = params, wsdl, operation) when is_tuple(params) do
    case validate_params(cdata, wsdl, operation) do
      {:error, _err} = errors -> errors
      _ -> params
    end
  end
  ...
  @spec validate_param(param :: tuple(), wsdl :: map(), operation :: String.t()) ::
          String.t() | nil
  ...
  @spec validate_param_attributes(val_map :: map(), k :: String.t(), v :: String.t()) ::
          String.t() | nil
  ...
  defp build_soap_body(wsdl, operation, params) do
    case params
         ...
         |> validate_params(wsdl, operation) do
      {:error, messages} ->
        {:error, messages}
  ...
  defp construct_xml_request_header(params) when is_atom(params) or is_number(params),
    do: params |> to_string
  ...
  defp add_action_tag_wrapper(body, %{nested_complex_types: _} = wsdl, operation) do
    action_tag_attributes = handle_element_form_default(wsdl[:schema_attributes])
    {action, nested_action, _type} = get_nested_action(wsdl, operation)

    action_tag = prepare_action_tag(wsdl, action, operation)
    nested_action_tag = prepare_action_tag(wsdl, nested_action, operation)

    [element(action_tag, action_tag_attributes, [element(nested_action_tag, nil, body)])]
  end
  ...
  defp handle_element_form_default(%{target_namespace: ns, element_form_default: "qualified"}),
    do: %{xmlns: ns}
  ...
  defp get_nested_action(wsdl, operation) do
    ...
    with types <- wsdl[:nested_complex_types],
         %{name: name, nested: %{name: nested_name, type: type}} <-
           Enum.find(types, find_by_name) do
      {name, nested_name, type}
    end
  end
  ...
  defp add_envelope_tag_wrapper(body, wsdl, operation) do
    envelop_attributes =
      ...
      |> Map.merge(service_namespaces(wsdl))
  ...
  defp service_namespaces(%{
         service: %{name: service_name},
         schema_attributes: %{target_namespace: location}
       }) do
    %{"xmlns:#{service_name}" => location}
  end

  defp service_namespaces(_wsdl), do: %{}
